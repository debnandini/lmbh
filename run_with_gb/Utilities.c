...
static void reverse_realft_wrapper(double *data, int n)
{
    
    /*
     Substitute reverse GSL radix2 FFTs.
     
     E.g.,
    
     Replace: gsl_fft_halfcomplex_radix2_inverse(data, 1, n);
     With:    reverse_realft_wrapper(data,n);
    
     Assumes that input arrays are packaged as if
     the radix2 functions were used so that
     no other changes for a radix2-based
     code are needed.
     */
    ...
    // repack arrays for realft
    for(int i=0; i<n; i++)    scratch[i] = data[i];
    for(int i=1; i<n/2; i++)
    {
        data[2*i-1] = scratch[i];
        data[2*i] = scratch[n-i];
    }
...
void clean(double *D, double *Draw, double *Hf, double *sqf, double *freqs, double *Sn, double *specD, double *sspecD, double df, double Q, double Tobs, double scale, double alpha, int Nf, int N, int imin, int imax, double *SNR, int pflag)
{
    ...
    */
    ...
    /* RealFT but using the same array paccking as radix2 */
    forward_realft_wrapper(D,N);
    forward_realft_wrapper(Dtemp,N);
    ...
    *SNR = S;
    ...
    /* RealFT */
    reverse_realft_wrapper(Dtemp,N);
...
void qscan(double *data, double *Sn, double Tobs, int N)
{
    ...
    */
    ...
    /* RealFT */
    forward_realft_wrapper(dcopy,N);
...
void phase_blind_time_shift(double *corr, double *corrf, double *data1, double *data2, int n)
{
    ...
    /* RealFT */
    reverse_realft_wrapper(corr,n);
    reverse_realft_wrapper(corrf,n);
