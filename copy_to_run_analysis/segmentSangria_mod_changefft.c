...
int main(int argc, char *argv[])
{
  ...
  FILE *out;
    ...
    days = atof(argv[1]);
    //x = floor(log2(rint(days*day/tdi->delta)));
    //y = pow(2.0,x);
    y = rint(days*day/tdi->delta);
    Tseg = y*tdi->delta;
    //x = Tseg/(days*day);
    //if(x < 0.75) y *= 2.0;
    Tseg = y*tdi->delta;
   ...
   gsl_fft_real_wavetable * real = gsl_fft_real_wavetable_alloc (Nseg);
   gsl_fft_real_workspace * work = gsl_fft_real_workspace_alloc (Nseg);
     ...
     for (i = 0; i < Nseg; ++i)
      {
       ...
       fclose(out);
          ...
          gsl_fft_real_transform (A, 1, Nseg, real, work);
          gsl_fft_real_transform (E, 1, Nseg, real, work);
          gsl_fft_real_transform (T, 1, Nseg, real, work);
